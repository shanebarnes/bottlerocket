...
/**
 * @brief Match a CLI argument with a CLI argument tuple.
 *
 * @param[in] argc A CLI argument count.
 * @param[in] argv A CLI argument array.
 * @param[in] argi A pointer to a CLI argument array index.
 *
 * @return A character representing the matched CLI tuple (0 on error).
 */
static char cli_options_getarg(const int32_t argc,
                               char ** const argv,
                               int32_t * argi)
{
    char retval = 0, c;
    ...
    else
    {
            ...
            (options[i+1].sname != 0))
        {
            ...
            if ((utilstring_compare(argv[*argi+1], prefix_skey, strlen(prefix_skey), true) == false) &&
                (utilstring_compare(argv[*argi+1], prefix_lkey, strlen(prefix_lkey), true) == false))
            {
                (*argi)++;
                        ...
                        struct cli_options_instance * const instance)
{
    ...
    if (instance != NULL)
    {
        ...
        for (i = 1; (i < argc) && (instance->exit == false); i++)
        {
            switch (cli_options_getarg(argc, argv, &i))
            {
#if !defined(__APPLE__)
                case 'A':
#endif
